...
- `/v1/redkeycluster/replicas`: endpoint to get and update the RedKey Cluster primary and replica nodes configuration. Methods provided:
  - `GET`: returns a JSON with the number of current RedKey Cluster primaries and replicas. The JSON contains thes field `primaries` (integer) and `replicas_per_primary` (integer).
  - `PUT`: updates the RedKey Cluster primaries and replicas. It expects a JSON with the field `primaries` (integer) and, optionally, `replicas_per_primary` (integer). Once received, Robin will modify the cluster to have the desired primaries and replicas per primary. 
...
- `/v1/cluster/nodes`: endpoint that returns the information that Robin has about the nodes. Methods provided:
  - `GET`: returns a JSON with a list of object, each containing `id` (string), `name` (string), `flags` (string), `slots` (array of objects with `start` (integer) and `end` (integer)), `ip` (string), `primaryId` (string), `failures` (integer), `sent` (integer), `recv` (integer) and `linkStatus` (string).
...
- `PUT` to `/v1/cluster/replicas`: Robin will change the primaries and replicas, launch a separate goroutine to perform the associated operations and returns.
...
- 200 OK: the request is already done. This response will depend on the called endpoint:
  - `/v1/cluster/replicas`: the RedKey Cluster has already the requested primaries and replicas.
  - `/v1/cluster/move`: the node received at `from` is already empty, `from` node is a replica or `from` node has replicas (and one of them has been promoted to primary).
